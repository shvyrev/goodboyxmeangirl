...
package railk.as3.motion.utils
{
	...
	public class Composite
	{
		...
		public function Composite(target:*,tween:*) {
			this.target = target;
			this.tween = tween;
		...
		public function start():void {
			if (!inited) init();
			if (state != 'end') { tween.start(); state='play'; }
		}
		...
		public function goto(pos):void {
			if ( pos != gotoPos ) {
				loop:for (var i:int = 0; i < timeline.length ; i++) {
					if (pos > timeline[i][0] && pos <= timeline[i][1]) {
						tween.setProps(timeline[i][2]);
						tween.setPosition(tween.delay + (pos - timeline[i][0]));
						//curPos=i;
						break loop;
					} else if (pos > timeline[i][1]) {
						tween.setProps(timeline[i][2]);
						tween.setPosition(tween.delay+(timeline[i][1]-timeline[i][0]));
					}
				}
				gotoPos = pos;
			}
