package railk.as3.tween.process.mode {
	//import railk.as3.tween.process.plugin.bezier.BesizePlugin;
	...
	public class AllMode implements IMode {
		private var manager:IPlugin;
		private var plugins:Array;
		public function AllMode():void { 
			manager = new PluginManager();
			plugins = [ new ColorizePlugin(), new BrightnessPlugin(), new HuePlugin(), new ContrastPlugin(), new SaturationPlugin(), new ThresholdPlugin(), new TintPlugin(), new TextPlugin(), new SoundPlugin(), new BevelFilterPlugin(), new BlurFilterPlugin(), new DropShadowFilterPlugin(), new GlowFilterPlugin(), /*new BezierPlugin()*/ ]
		}
