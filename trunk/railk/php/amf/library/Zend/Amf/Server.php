...
class Zend_Amf_Server implements Zend_Server_Interface
{
   ...
   /**
	...
	protected function _handleAuth( $userid,  $password)
	{
		...
		$auth = Zend_Auth::getInstance();
		$auth->setStorage(new Zend_Auth_Storage_Session($_sessionNameSpace));
    ...
    protected function _handle(Zend_Amf_Request $request)
    {
        ...
        $handleAuth = false;
		if ($this->_auth) {
			$headers = $request->getCredentials(); //getAmfHeaders();
			if ( $headers && isset($headers->userid) ) {
				$handleAuth = true;
			}
		}
        ...
        foreach($responseBody as $body)
        {
            try {
				if ($handleAuth) {
					if ($this->_handleAuth($headers->userid, $headers->password)) {
